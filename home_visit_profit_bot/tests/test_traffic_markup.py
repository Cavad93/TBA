...
пробок ≥ 1,30. А в план дня уходит EMA, ЗАСЕЯННАЯ дефолтом 2,0 — то есть надбавку платил
КАЖДЫЙ, включая человека, который ещё ни одной смены не закрыл и про чьи пробки мы не
знаем ничего. Дефолт — это наше предположение, а не его факт; выдавать предположение за
измеренный факт и брать за него деньги нельзя.

Теперь в надбавку идёт ЧИСТОЕ среднее фактических коэффициентов по закрытым сменам, без
дефолтного посева, и не раньше трёх смен: одна смена — это совпадение, а не пробки.
"""
