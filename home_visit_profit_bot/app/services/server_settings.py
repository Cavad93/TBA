...
def travelpayouts_token() -> str | None:
    """Ключ Data API Travelpayouts — цены билетов (Ф11.6). ТОЛЬКО из окружения сервера.

    Как и у DaData: ключ из APK извлекается, а привязки к приложению у него нет — значит
    любой мог бы жечь наш лимит и подставлять наш маркер. Нет ключа → блок билетов молчит
    (`tickets_service.tickets_block` без токена возвращает None). Выдуманных цен не бывает.
    """
    return os.getenv("TRAVELPAYOUTS_TOKEN") or None


def travelpayouts_marker() -> str:
    """Партнёрский маркер Travelpayouts — им метится ссылка, по нему считается доход.

    Не секрет (виден в любой партнёрской ссылке), но живёт рядом с токеном: пустой маркер
    не ломает блок, просто переход не засчитается — поэтому ссылка без маркера бесполезна,
    а не опасна.
    """
    return os.getenv("TRAVELPAYOUTS_MARKER", "")


def tickets_min_distance_km() -> float:
    """С какого расстояния (в одну сторону) вообще заводить разговор о билетах.

    Решение Джавада: только межгород >200 км. На 50 км самолёта не бывает, и лезть
    в API на каждую городскую поездку незачем — это лишний сетевой поход в горячем пути.
    """
    raw = os.getenv("TICKETS_MIN_DISTANCE_KM")
    if raw:
        try:
...
def tickets_savings_threshold() -> float:
    """Насколько билет должен быть дешевле машины, чтобы вообще показать блок (доля).

    Дефолт 0.10 — почти равная цена это шум, а не информация. Настройкой правится без
    релиза: если окажется, что 10% мало (человек всё равно едет на машине из-за багажа),
    порог поднимаем данными, а не спором.
    """
    raw = os.getenv("TICKETS_SAVINGS_THRESHOLD")
    if raw:
        try:
