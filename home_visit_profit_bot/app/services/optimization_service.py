...
def optimize_route(
    ...
    *,
    ...
    matrix = cached_distance_matrix(
        points,
        osrm_url=osrm_url,
        # Профиль ОБЯЗАН доехать и сюда. Он задаёт радиус привязки к дороге: у машины
        # 2000 м, у пешехода и велосипеда 1000 м. С дефолтным «driving» пешеход молча
        # получал автомобильный радиус — точка в двух километрах считалась достижимой,
        # и «вне покрытия» не срабатывало там, где должно. В ветке «нет заказов» ниже
        # профиль передавался, в основной — нет.
        profile=profile,
        timeout_seconds=timeout_seconds,
        duration_factor=duration_factor,
    )
