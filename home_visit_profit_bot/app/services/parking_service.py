...
class ParkingHit:
    ...
    paid_now: bool
    # Цена из открытых данных города — текстом, а не числом: она бывает и не
    # почасовой («первые 30 мин — 50 ₽, дальше 150 ₽ до конца дня»).
    city_price: str | None = None

    def with_price(self, price: str | None) -> "ParkingHit":
        return ParkingHit(
            zone=self.zone,
            tariff=self.tariff,
...
def is_municipal(zone: ParkingZone) -> bool:
    """Городская платная зона — та, за которую платят в приложении города.

    Отчёт 922: в Приморском районе Петербурга приложение требовало оплатить парковку,
    хотя городской платной зоны там нет ни одной. Проверка по боевым данным подтвердила
    дословно: в рамке района 229 зон, все до одной `lot`, ни одной `street`, ни у одной
    нет кода зоны. Это коммерческие стоянки — у торговых центров, у бизнес-центров, и
    среди них буквально «Штрафстоянка». В базу они попадают законно: у них `fee=yes`,
    они действительно платные. Ошибка не в данных, а в том, что мы считали любую платную
    парковку городской зоной и приписывали ей городской тариф и городское приложение.

    Различаем так:
      * `kind="street"` — парковка вдоль улицы. Это по определению городская: у обочины
        частных стоянок не бывает;
      * код зоны (КЗ-2 в Петербурге, номер в Москве) — прямая метка городской разметки,
        и у плоскостных городских парковок он есть.

    Коммерческая стоянка под это правило не подходит — и правильно: там шлагбаум или
    касса, наше уведомление человеку ничего не даёт, а совет «оплатите в приложении
    парковки» прямо неверен.
    """
    return zone.kind == "street" or bool(zone.zone_code)
