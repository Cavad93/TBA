...
SETTINGS_CATALOG: list[SettingField] = [
    ...
    # Фикс-расходы смены (Фаза 10). У кого свой авто — оставляет 0, и блок аренды молчит.
    # По ним считается точка безубыточности: когда чистый доход дня перекрыл обязательные
    # траты, смена «отбита» — дальше работаешь на себя.
    SettingField(
        "shift_rent_cost", "economics", "Аренда авто, ₽ за смену", "number", 0.0, min=0,
        hint="Сколько платите за аренду машины в день. Свой авто — оставьте 0.",
    ),
    SettingField(
        "shift_fixed_costs", "economics", "Прочие обязательные расходы смены, ₽", "number", 0.0, min=0,
        hint="Мойка, связь и другие траты, которые есть в каждую смену. Входят в точку безубыточности.",
    ),
    ...
    SettingField(
        # «Открывать навигатор сам» читалось двояко: сам — приложение или ты сам?
        # Называем действующее лицо прямо (отчёт 854 из TG).
        "auto_open_navigator", "navigation", "Приложение открывает навигатор само", "bool", False,
        hint=(
            "Включено: после закрытия заказа приложение само откроет навигатор на следующий "
            "адрес, показав отсчёт — успеете отменить. Выключено: навигатор открываете вы "
            "кнопкой «Поехали»."
        ),
    ),
    SettingField(
    ...
    SettingField(
        # «Закрывать заказ самому» читалось ПРЯМО НАОБОРОТ: будто закрываешь ты сам,
        # тогда как настройка отдаёт закрытие приложению (отчёт 854 из TG).
        "auto_close_visit", "navigation", "Приложение закрывает заказ само", "bool", False,
        hint=(
            "Включено: если вы простояли у адреса дольше, чем указано в «Долгая остановка», "
            "заказ закроется без вашего участия (закрытие можно отменить). Выключено: заказ "
            "закрываете вы кнопкой «Готово»."
        ),
    ),
    # GPS
    SettingField(
    ...
    # Одометр по личным поездкам (Фаза 6). По умолчанию ВЫКЛ. Точки вне смены идут
    # ТОЛЬКО в километраж авто (точнее износ) — не создают заказов, не участвуют в
    # парковке, телеметрии и индексах. Хранятся 14 дней, как весь GPS. Согласие на
    # геолокацию (152-ФЗ, обычные ПДн) фиксирует экран включения на телефоне.
    SettingField(
        "count_personal_trips", "gps", "Считать личные поездки в пробеге авто", "bool", False,
        hint="Вне смены приложение изредка отмечает километраж — для точного износа. "
             "Только пробег: никаких заказов и выводов о вас. Хранится 14 дней, выключается тут же.",
    ),
    # Нагрузка
    SettingField(
