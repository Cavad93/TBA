...
OVERWORK_EXTRA_WEIGHTS: dict[str, float] = {
    "workload_rating": 0.45,     # «насколько загруженной была смена» — оценка условий труда
    "overtime_minutes": 0.35,    # сверхурочные сверх личной нормы часов
    "break_interrupted": 0.20,   # перерыв между сменами прерывался
}
