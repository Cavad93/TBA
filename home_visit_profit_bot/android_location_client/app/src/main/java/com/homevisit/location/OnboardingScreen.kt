...
/**
 * Онбординг после регистрации: проводим по ВСЕМ секциям настроек (Этап 26,
 * поручение владельца). Смысл — не анкета ради анкеты: себестоимость километра,
 * планка ₽/час, тип занятости и зоны — это входы каждого вердикта «стоит ли
 * ехать». Без них расчёт опирается на средние значения и советует абстрактно.
 *
 * Каталог секций и полей приходит с сервера (тот же, что в Настройках), рендер —
 * тот же AppSettingsSection: у каждого поля своё серверное пояснение. Пропустить
 * можно (человек может не знать цифр в момент регистрации), но с честным
 * предупреждением о том, чем это обернётся.
 */
        ...
        Column(
            ...
            when {
                ...
                page <= sections.size -> {
                    ...
                    Text(
                        "Эти параметры напрямую входят в расчёт каждого заказа. " +
                            "Заполните, что знаете, — остальное можно уточнить позже в Настройках.",
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        Button(modifier = Modifier.weight(1f), onClick = { page += 1 }) {
                            Text("Дальше")
                        }
                    }
                }
                    ...
                    Text(
                        "Сохраняем ответы — с этого момента вердикты считаются по вашим " +
...
private fun IntroPage(onStart: () -> Unit, onSkip: () -> Unit) {
    Text(
        "Пара минут — и советы станут вашими",
        style = MaterialTheme.typography.titleLarge,
        fontWeight = FontWeight.SemiBold,
    )
    Text(
        "Сейчас приложение спросит то, из чего складывается каждый вердикт «стоит ли " +
            "ехать»: сколько стоит ваш километр, ниже какой планки ₽/час заказ не " +
            "интересен, как вы зарабатываете и на чём ездите.",
        style = MaterialTheme.typography.bodyMedium,
    )
    Text(
        "Это важно: без ваших цифр расчёты опираются на средние значения — " +
            "рекомендации будут абстрактными и могут советовать невыгодное. " +
            "С вашими ответами каждый вердикт персональный.",
        style = MaterialTheme.typography.bodyMedium,
    )
    Text(
        "Не знаете какое-то значение — оставьте как есть и уточните позже: всё это " +
            "меняется в Настройках в любой момент.",
        style = MaterialTheme.typography.bodySmall,
        color = MaterialTheme.colorScheme.onSurfaceVariant,
    )
