...
object ReminderMessages {
    ...
    /**
     * Алерт «смена уходит в минус» (Ф10.3): середина смены пройдена, накопленный чистый
     * отрицательный. Кричим днём, а не вечером — чтобы человек успел пересмотреть. Тон
     * заботы, не паника. null — если смена не в минусе (не тревожим).
     */
    fun negativeShift(accumulatedNet: Double): Message? {
        if (accumulatedNet >= 0) return null
        return Message(
            title = "Смена пока в минусе",
            body = "Сейчас ${accumulatedNet.roundToInt()} ₽ чистыми. Стоит присмотреться, какие заказы брать дальше.",
        )
    }
